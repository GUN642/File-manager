...
/** Checks the GitHub releases of this app and downloads new APKs. */
object Updater {
    private const val REPO = "GUN642/File-manager"
    private const val API = "https://api.github.com/repos/$REPO/releases?per_page=30"

    /** Parses "v1.0.12" / "1.0.12" into comparable numbers. */
    ...
    private fun open(url: String): HttpURLConnection {
        ...
        conn.setRequestProperty("User-Agent", "VOID-Files")
        return conn
    }

    /** All published releases, newest version first. */
        ...
        try {
            ...
            for (i in 0 until json.length()) {
                ...
                out += Release(
                    ...
                    notes = cleanNotes(tag.removePrefix("v"), r.optString("body")),
    ...
    /** Notes for releases published before release bodies carried a changelog. */
    private val knownNotes = mapOf(
        "1.0.1" to "- Erste Version von VOID Files\n- Dateiverwaltung, Papierkorb, Archive, Themes\n" +
            "- Zwei Fenster im Querformat\n- Proton-Drive-Integration",
        "1.0.3" to "- Ordnerpfad in Akzentfarbe\n- Schnellzugriff bearbeitbar (hinzufügen, entfernen, sortieren)\n" +
            "- Neuer Schnellzugriff \"Letzte Dateien\"",
    )

    /** Keeps only the human readable change notes from a release body. */
    private fun cleanNotes(version: String, body: String): String {
        if (body.isBlank() || body.startsWith("Automatischer Build")) return knownNotes[version].orEmpty()
        return body.lines()
            .filterNot { it.startsWith("Co-Authored-By", ignoreCase = true) || it.startsWith("Claude-Session") }
            .joinToString("\n").trim()
    }
